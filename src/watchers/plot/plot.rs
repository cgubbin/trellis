...
pub fn plot_csv(
    ...
    plot_kind(&rows, output_dir.as_ref(), PlotKind::Metric)?;
    ...
    match kind {
        PlotKind::Metric => {
            ...
            for (idx, (tag, values)) in series.iter().enumerate() {
                let colour = Palette99::pick(idx);

                chart
                    .draw_series(LineSeries::new(values.iter().copied(), &colour))?
                    .label(tag.clone())
        ...
        PlotKind::Absolute | PlotKind::Relative => {
            ...
            let mut chart = ChartBuilder::on(&root)
                ...
                .x_label_area_size(40)
                .y_label_area_size(70)
                .build_cartesian_2d(0usize..x_max.max(1), (y_min.log10())..(y_max.log10()))?;
            ...
            for (idx, (tag, values)) in series.iter().enumerate() {
                ...
                chart
                    .draw_series(LineSeries::new(transformed, &colour))?
                    .label(tag.clone())
                    .legend(move |(x, y)| PathElement::new(vec![(x, y), (x + 20, y)], &colour));
            }
